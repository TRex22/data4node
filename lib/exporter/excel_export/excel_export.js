...
var excelData = require('./excelData.js');
var stylizer = require('../../stylizer/stylizer.js');
...
var exposed = {
  setConfig: setConfig,
  makeExcelDocument: makeExcelDocument
};
...
function makeExcelDocument(data, styleObj, file) {
  var wb = new xl.WorkBook();
  wb.debug = config.internalWbLibDebug;

  var dataCells = mrData.getJsonData(data, styleObj);
  var worksheets = excelData.getWorksheets(wb, dataCells, styleObj);

  helper.log("------------Raw Data Done------------");

  //// run stylizer
  //if (styleObj) {
  //  stylizer.shazam(dataCells, styleObj, wb, worksheets);
  //} else {
...
function makeExcelTimeTable(times, styles, file){
  var wb = new xl.WorkBook();
  wb.debug = config.internalWbLibDebug;
  ...
  var worksheets = mrData.getWorksheets(wb, data, styleObj, cells);

  helper.log("------------Raw Data Done------------");

  // run stylizer
  if (styleObj) {
    stylizer.createTable(data, styleObj, wb, worksheets, cells);
  } else {
    helper.log("Styles Object is Empty."); //TODO JMC add try-catches
  }
