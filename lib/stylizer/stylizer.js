...
var typeCast = require('../exporter/excel_export/typeCast.js');
...
function shazam(dataCells, styleObj, wb, worksheets) {
  var custStyles = getCustomStyles(styleObj, wb);
  setCustStyles(worksheets, dataCells, styleObj, custStyles);
...
function setCustStyles(worksheets, dataCells, styleObj, custStyles) {
  //set batch cells first and then follow individual cell mods
  helper.log("------------Starting Batch Styling------------");
  setBatchHeadingStyles(worksheets, styleObj, custStyles, dataCells);
  setBatchDataStyles(worksheets, styleObj, custStyles, dataCells);
  setColProperties(worksheets, dataCells, styleObj, custStyles);
  helper.log("------------Batch Styling Done----------------");
  setCustCellStyles(worksheets, styleObj, custStyles, dataCells);
  helper.log("------------Custom Styles Objects Set------------");
}

function setBatchHeadingStyles(worksheets, styleObj, custStyles, cells) {
  helper.log(util.inspect(cells));
    ...
    for (var i = 0; i < styleObj.data.colStyles.length; i++) {
      ...
      if (style) {
        ...
        if (custStyle) {
          ...
          for (var j = 0; j < cells.data.length; j++) { //TODO FIX ws wrong
            var row = (j + 1);
      ...
      if (type) {
        for (var j = 0; j < cells.data.length; j++) {
          var row = (j + 2);
          typeCast.convert(type, col, row, ws, worksheets, numberFormat,
            cells);
        }
