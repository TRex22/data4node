...
//the data obj here is the raw data obj, mrData will place it in the whole object thing
//this object should be one set of str csv data
...
function parseToCsv(data, useHeader) {
  var str = "";
  if (useHeader) { //if useHeader is true then the inital line is taken from properties name
    data.getOwnPropertyNames(obj).forEach(function(prop, idx, array) {
      if (prop !== "length")
        str = helper.commaConcat(str, prop);
    });
  }
